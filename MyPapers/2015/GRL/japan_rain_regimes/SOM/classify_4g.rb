...
IO.readlines(som_map)[1..-1].each do |line|
  line.chomp!
  data << line.split.map {|col| (1+(col.to_f/0.33).floor).to_i}
end
...
(1..ydim.to_i).each do |j|
  (1..xdim.to_i).each do |i|
    node=data[k] 
    if ((node[0]>node[1]) and (node[0]>node[2]) and (node[0]>node[3]))
       idec << "#{i-1}_#{j-1}"
      p node.join(",")
    end
    k+=1
...
(1..ydim.to_i).each do |j|
  (1..xdim.to_i).each do |i|
    node=data[k] 
    if ((node[1] > node[0]) and (node[1] > node[2]) and (node[1]>node[3]))
       dec << "#{i-1}_#{j-1}"
       p node.join(",")
    end
    k+=1
...
(1..ydim.to_i).each do |j|
  (1..xdim.to_i).each do |i|
    node=data[k] 
    #puts node.join(" ")
    if ((node[2] > node[1]) and (node[2] > node[0])  and (node[2]>node[3]))
       int << "#{i-1}_#{j-1}"
       p node.join(",")
    end
    k+=1
...
(1..ydim.to_i).each do |j|
  (1..xdim.to_i).each do |i|
    node=data[k] 
    #puts node.join(" ")
    if ((node[3] > node[2]) and (node[3] > node[1]) and (node[3]>node[0]))
       qbo << "#{i-1}_#{j-1}"
    end
    k+=1
  end 
...
File.open("typ1_nodes.txt","w") {|f| f.puts idec}
File.open("typ2_nodes.txt","w") {|f| f.puts dec}
File.open("typ3_nodes.txt","w") {|f| f.puts int}
File.open("typ4_nodes.txt","w") {|f| f.puts qbo}
